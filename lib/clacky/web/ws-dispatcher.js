  ...
  switch (ev.type) {
    ...
    case "input_queue": {
      ...
      for (const entry of ev.entries || []) {
        ...
        status.textContent = I18n.t("chat.input.queued");
        ...
        sendNow.onclick = () => {
          if (WS.ready) WS.send({ type: "send_pending_input", session_id: ev.session_id, id: entry.id, lang: I18n.lang() });
        };
        actions.append(edit, sendNow, remove);
    ...
    case "history_user_message":
      if (ev.steering && ev.session_id === Sessions.activeId) {
        Sessions.appendMsg("user", escapeHtml(ev.content || "") + (ev.images || []).map(value =>
          value.startsWith("pdf:") ? "<br>" + escapeHtml(value.slice(4)) : `<br><img class="msg-image-thumb" src="${escapeHtml(value)}">`).join(""));
      }
    ...
    case "assistant_message":
      ...
      Sessions.appendAssistantMessage(ev.content, {
    ...
    case "tool_result":
      if (ev.session_id !== Sessions.activeId) break;
      Sessions.appendToolResult(ev.result, ev.tool_call_id, {
