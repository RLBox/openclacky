...
const Settings = (() => {
  ...
  async function _refreshRuntimeCardStatus(provider, model, index) {
    ...
    const data = await RuntimeProvider.status(provider);
    if (!isCurrent()) return data;
    const view = RuntimeProvider.statusView(data);
    render(data, view);
    if (view.terminal) return data;
    ...
    if (isCurrent()) render(result, result.view);
    return result;
  }
  ...
  async function _refreshModalRuntimeStatus(provider = _selectedModalProvider()) {
    ...
    const data = await RuntimeProvider.connect(provider);
    if (!isCurrent()) return data;
    const view = _renderModalRuntimeStatus(data);
    if (view.terminal) return data;
    ...
    const result = await RuntimeProvider.pollStatus(provider, {
      ...
      onUpdate: (nextData, nextView) => {
        if (isCurrent()) {
          _renderModalRuntimeStatus(nextData, nextView);
        }
      },
    });
    if (isCurrent()) {
  ...
  async function _saveRuntimeModalModel(provider, index, saveBtn) {
    ...
    const testResult = await ModelTester.testRuntime({
      provider_id: provider.id,
      id: existingId || undefined,
      index: existingId ? index : undefined,
    });
