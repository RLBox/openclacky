...
const Settings = (() => {
  ...
  async function _load() {
    ...
    try {
      ...
      _personalByokAllowed = configData.personal_byok_allowed !== false;
      _renderPersonalByokPolicy();
      _renderCards();
    } catch (e) {
  ...
  function _renderPersonalByokPolicy() {
    const addButton = document.getElementById("btn-add-model");
    const notice = document.getElementById("personal-byok-policy-notice");
    if (addButton) addButton.style.display = _personalByokAllowed ? "" : "none";
    if (notice) notice.style.display = _personalByokAllowed ? "none" : "block";
  }
  ...
  function _renderCard(container, model, index) {
    ...
    const isEnterpriseManaged = model.enterprise_managed === true;
    ...
    const providerName = isEnterpriseManaged
      ? I18n.t("settings.models.badge.enterpriseManaged")
    ...
    card.innerHTML = `
      <div class="model-card-grid-info">
        <div class="model-card-grid-name-row">
          ...
          ${!isDefault && !isEnterpriseManaged ? `<button class="btn-card-grid-action btn-card-grid-action-primary" data-index="${index}" data-action="default" style="margin-left:auto">
      ...
      <div class="model-card-grid-actions">
        <div class="model-card-grid-toolbar">
          ...
          ${!isEnterpriseManaged ? `<button class="btn-card-grid-action" data-index="${index}" data-action="edit">
          ...
          ${!isEnterpriseManaged && _models.length > 1 ? `<button class="btn-card-grid-action btn-card-grid-action-danger" data-index="${index}" data-action="delete">
  ...
  function _syncApiFormatVisibility() {
    ...
    if (field) field.style.display = _modalProviderIsCustom ? "" : "none";
  }
  ...
  async function _loadBrand() {
    try {
      ...
      if (!data.enterprise_license) {
        try {
          const enterpriseRes = await fetch("/api/enterprise/license");
          if (enterpriseRes.ok) data.enterprise_license = await enterpriseRes.json();
        } catch (_) { /* Older clients may not expose enterprise status. */ }
      }
  ...
  function _renderBrandStatus(data) {
    ...
    const enterprise = data.enterprise_license;
    const enterpriseCard = document.getElementById("enterprise-license-card");
    if (enterpriseCard) enterpriseCard.style.display = enterprise && enterprise.bound ? "" : "none";
    if (enterprise && enterprise.bound && enterpriseCard) {
      statusCard.style.display = "none";
      activateForm.style.display = "none";
      document.getElementById("enterprise-license-name").textContent = enterprise.product_name || I18n.t("settings.enterprise.title");
      document.getElementById("enterprise-license-state").textContent = I18n.t(
        enterprise.active ? "settings.enterprise.active" :
          enterprise.reason === "unreachable" ? "settings.enterprise.unreachable" : "settings.enterprise.inactive"
      );
      document.getElementById("enterprise-license-expires").textContent = enterprise.active ?
        (enterprise.expires_at ? new Date(enterprise.expires_at).toLocaleDateString() : I18n.t("settings.enterprise.permanent")) : "—";
      document.getElementById("enterprise-license-devices").textContent = enterprise.active ?
        `${enterprise.devices_used} / ${enterprise.device_limit}` : "—";
      return;
    }
  ...
  function _renderMediaRow(kind) {
    ...
    ["off", "auto", "custom"].forEach(src => {
      if (src === "custom" && !_personalByokAllowed) return;
  ...
  function init() {
    ...
    document.getElementById("btn-refresh-enterprise-models")?.addEventListener("click", async (event) => {
      const button = event.currentTarget;
      const status = document.getElementById("enterprise-model-refresh-status");
      button.disabled = true;
      button.textContent = I18n.t("settings.enterprise.models.refreshing");
      status.textContent = "";
      status.className = "enterprise-model-refresh-status";
      const result = await EnterpriseModels.refresh({ force: true });
      if (result.ok) {
        status.textContent = I18n.t(
          result.changed ? "settings.enterprise.models.updated" : "settings.enterprise.models.current",
          { count: result.model_count || 0 }
        );
        status.classList.add("success");
        if (result.changed) await _load();
      } else {
        status.textContent = I18n.t("settings.enterprise.models.failed");
        status.classList.add("error");
      }
      button.disabled = false;
      button.textContent = I18n.t("settings.enterprise.models.refresh");
    });
