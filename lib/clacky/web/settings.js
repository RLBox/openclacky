...
const Settings = (() => {
  ...
  let _modalRuntimeModels = [];
  let _modalRuntimeStatusData = null;
  let _runtimeCardStatusGeneration = 0;
  let _runtimeCardStatusRequest = 0;
  const _runtimeCardStatusRequests = new Map();
  let _modelSaveRequest = 0;
  let _modelSaveInFlight = false;
  ...
  function _renderCards() {
    ++_runtimeCardStatusGeneration;
    _runtimeCardStatusRequests.clear();
    const container = document.getElementById("model-cards");
    container.innerHTML = "";
    ...
    const query = _modelSearchQuery.trim().toLowerCase();
    let filtered = _models.slice();
    if (_modelProviderFilter) {
      filtered = filtered.filter(m => _providerFilterKey(m) === _modelProviderFilter);
    }
    if (query) {
      filtered = filtered.filter(m => _matchesModelSearch(m, query));
    }

    if (filtered.length === 0) {
      container.innerHTML = `<div class="settings-empty">${I18n.t("settings.models.search.noResults")}</div>`;
      return;
    }

    filtered.forEach(m => _renderCard(container, m, _models.indexOf(m)));
  }
  ...
  function _renderProviderFilter() {
    ...
    const names  = new Map();
    _models.forEach(m => {
      const key = _providerFilterKey(m);
      counts.set(key, (counts.get(key) || 0) + 1);
      if (!names.has(key)) {
        const p = _getProvider(m);
        names.set(key, p ? _providerDisplayName(p) : I18n.t("settings.models.provider.custom"));
      }
    });
  ...
  function _matchesModelSearch(model, query) {
    const haystack = [
      model.model,
      model.display_model,
      model.remark,
      model.base_url,
      _getProviderName(model)
  ...
  // The remark input is always shown regardless of provider; the typed value
  // is kept so toggling providers never silently drops it. The no-vision
  // override only applies to custom providers - preset capability tables live
  // server-side, so no per-model override is needed there.
  function _syncRemarkField() {
    const input = document.getElementById("model-modal-remark");
    if (input) {
      const field = input.closest(".model-field");
      if (field) field.style.display = "";
    }

    const noVision = document.getElementById("model-modal-no-vision-field");
    if (noVision) noVision.style.display = _modalProviderIsCustom ? "" : "none";
  }

  function _getProviderName(model) {
    return _providerDisplayName(_getProvider(model));
  }
  ...
  function _getProvider(model) {
    ...
    if (model.provider_id) {
      const provider = _providers.find(pr => pr.id === model.provider_id) || null;
      if (provider) {
        const runtimeProvider = RuntimeProvider.isRuntimeProvider(provider);
        if (!runtimeProvider && !model.runtime_id) return provider;
        if (runtimeProvider && model.runtime_id === provider.runtime_id) return provider;
      }
      if (model.runtime_id) {
        return {
          ...
          display_model: model.display_model,
          extension_id: null,
          unavailable: true,
        };
      }
  ...
  // Preset providers are keyed by id; unmatched (custom base_url) models
  // share the "custom" bucket.
  function _providerFilterKey(model) {
    const p = _getProvider(model);
  ...
  function _renderCard(container, model, index) {
    ...
    const provider  = _getProvider(model);
    const runtimeAvailable = model.runtime_available !== false;
    ...
    const displayName = RuntimeProvider.displayModel(model, provider) || I18n.t("settings.models.unnamed");
    ...
    card.innerHTML = `
      <div class="model-card-grid-info">
        <div class="model-card-grid-name-row">
          ...
          ${!isDefault && runtimeAvailable ? `<button class="btn-card-grid-action btn-card-grid-action-primary" data-index="${index}" data-action="default" style="margin-left:auto">
        ...
        ${remark ? `<div class="model-card-grid-remark" title="${_esc(remark)}">${_esc(remark)}</div>` : ""}
        ${!RuntimeProvider.isRuntimeProvider(provider) && model.api_key_masked ? `<div class="model-card-grid-model">${_esc(model.api_key_masked)}</div>` : ""}
        <div class="model-card-grid-status">
          <span class="model-test-result" data-index="${index}">${RuntimeProvider.isRuntimeProvider(provider) ? _esc(I18n.t("runtime.provider.status.checking")) : ""}</span>
        </div>
      </div>
      <div class="model-card-grid-actions">
        <div class="model-card-grid-toolbar">
          ...
          ${!RuntimeProvider.isRuntimeProvider(provider) ? `<button class="btn-card-grid-action" data-index="${index}" data-action="duplicate">
    ...
    if (RuntimeProvider.isRuntimeProvider(provider)) {
      _refreshRuntimeCardStatus(provider, model, index);
    }
  }

  async function _refreshRuntimeCardStatus(provider, model, index) {
    const generation = _runtimeCardStatusGeneration;
    const request = ++_runtimeCardStatusRequest;
    _runtimeCardStatusRequests.set(model.id, request);

    const isCurrent = () => !(
      generation !== _runtimeCardStatusGeneration ||
      _runtimeCardStatusRequests.get(model.id) !== request ||
      _models[index]?.id !== model.id
    );
    const render = (data, suppliedView) => {
      if (!isCurrent()) return;
      ...
      el.textContent = `${view.connected ? "✓" : "•"} ${_runtimeStatusText(view, data)}`;
      el.className = `model-test-result ${view.connected ? "result-ok" : (view.state === "unavailable" ? "result-fail" : "result-testing")}`;
    };

    let data = await RuntimeProvider.connect(provider);
    if (!isCurrent()) return data;
    let view = RuntimeProvider.statusView(data);
    render(data, view);
    if (!view.connected && !view.terminal) {
      data = await RuntimeProvider.pollStatus(provider, {
        ...
        onUpdate: (nextData, nextView) => render(nextData, nextView),
      });
      if (!isCurrent()) return data;
      view = data.view || RuntimeProvider.statusView(data);
      render(data, view);
    }
    if (!view.connected) return data;

    const discovery = await RuntimeProvider.discover(provider);
    if (!isCurrent()) return discovery;
    render(discovery, RuntimeProvider.statusView(discovery));
    const legacyDefault = ["ChatGPT default", "Codex default"].includes(model.display_model);
    if (discovery.ok && legacyDefault && discovery.default_model) {
      const migrated = await ModelTester.saveModel(
        { display_model: discovery.default_model },
        { existingId: model.id }
      );
      if (isCurrent() && migrated.ok) {
        model.display_model = discovery.default_model;
        _load();
      }
    }
    return discovery;
  ...
  function _openModal(index = -1) {
    ...
    if (index >= 0 && _models[index]) {
      ...
      document.getElementById("model-modal-model").value = model.model || model.display_model || "";
      ...
      // A default is moved by promoting another card, never by leaving the
      // configuration without one. Keep the current default (and the sole
      // model) checked and locked so save cannot create an ambiguous state.
      const setDefaultCb = document.getElementById("model-modal-set-default");
      const isOnlyModel = _models.length === 1;
      const isCurrentDefault = model.type === "default";
      setDefaultCb.checked = isOnlyModel || isCurrentDefault;
      setDefaultCb.disabled = isOnlyModel || isCurrentDefault;
      ...
      const resolvedProvider = resolvedProviderId ? _getProvider(model) : null;
      ...
      document.getElementById("model-modal-remark").value = model.remark || "";
      const noVision = document.getElementById("model-modal-no-vision");
      noVision.checked = !!(model.capabilities && model.capabilities.vision === false);
      _syncApiFormatVisibility();
      _syncRemarkField();
      _syncModalProviderMode(resolvedProvider);
    } else {
      // Add mode
      ...
      _modalProviderIsCustom = false;
      _syncApiFormatVisibility();
      _syncRemarkField();
      _syncModalProviderMode(null);
      const providerValue = document.getElementById("model-modal-provider-value");
    ...
    // Keep the promotional hint tied to its provider even when editing.
    const promoHint = document.getElementById("model-modal-promo-hint");
    if (_modalSelectedProviderId === "openclacky" || !_modalSelectedProviderId) {
  ...
  function _closeModal(force = false) {
    if (_modelSaveInFlight && force !== true) return;
    _modelSaveRequest += 1;
  ...
  function _openModalDuplicate(index) {
    ...
    if (!source) return;
    if (RuntimeProvider.isRuntimeProvider(_getProvider(source))) return;
    ...
    const matched = resolvedProviderId
      ...
      : null;
    if (matched) {
      providerValue.textContent = _providerDisplayName(matched);
      providerValue.classList.remove("placeholder");

      const promoHint = document.getElementById("model-modal-promo-hint");
      if (matched.id !== "openclacky") promoHint.classList.remove("visible");
    } else {
      // Custom / anthropic-format sources previously kept the dropdown stuck
      // on the placeholder; resolve it so the state (and remark field) matches
      // what saving will actually produce.
      const fallback = _modalSelectedProviderId
        ? _providers.find(p => p.id === _modalSelectedProviderId) || null
        : null;
      providerValue.textContent = _providerDisplayName(fallback);
      providerValue.classList.remove("placeholder");
    }
    if (RuntimeProvider.isRuntimeProvider(matched)) _clearModalApiFields();
    _syncApiFormatVisibility();
    _syncRemarkField();
    _syncModalProviderMode(matched);
  }

  // API Format only applies to custom providers: preset routing is owned by
  // the backend (provider preset + per-model overrides), and the placeholder
  // state means the user hasn't committed to custom either - so the override
  // dropdown is visible only after an explicit "custom" provider choice.
  function _syncApiFormatVisibility() {
    const field = document.getElementById("model-modal-apiformat-field");
    const runtime = RuntimeProvider.isRuntimeProvider(_selectedModalProvider());
    if (field) field.style.display = _modalProviderIsCustom && !runtime ? "" : "none";
  }

  function _selectedModalProvider() {
    if (!_modalSelectedProviderId) return null;
    const registered = _providers.find(p => p.id === _modalSelectedProviderId) || null;
    if (registered) return registered;
    const index = parseInt(document.getElementById("model-modal-index")?.value, 10);
    const model = Number.isInteger(index) ? _models[index] : null;
    return model && model.provider_id === _modalSelectedProviderId
      ? _getProvider(model)
      : null;
  }

  function _modalProviderKey() {
    return _modalProviderIsCustom ? "custom" : (_modalSelectedProviderId || "");
  }

  function _clearModalApiFields() {
    document.getElementById("model-modal-model").value = "";
    document.getElementById("model-modal-baseurl").value = "";
    document.getElementById("model-modal-apikey").value = "";
  ...
  function _runtimeStatusText(view, data) {
    ...
    const detail = data && (data.label || data.message || data.error);
    return detail && detail !== translated ? `${translated} — ${detail}` : translated;
  }

  function _renderModalRuntimeStatus(data, suppliedView) {
    _modalRuntimeStatusData = data;
    const view = suppliedView || RuntimeProvider.statusView(data);
    const status = document.getElementById("model-modal-runtime-status");
    const login = document.getElementById("model-modal-runtime-login");
    const recheck = document.getElementById("model-modal-runtime-recheck");
    if (status) {
    ...
    if (login) {
      ...
      const canInstall = data?.can_install === true;
      login.textContent = I18n.t(canInstall ? "runtime.provider.install" : "runtime.provider.connect");
      login.style.display = !view.connected && (canInstall || canAuthenticate) && !["checking", "starting"].includes(view.state)
        ? ""
        : "none";
      login.disabled = _modelSaveInFlight || view.state === "starting";
    }
    if (recheck) recheck.disabled = _modelSaveInFlight || ["checking", "starting"].includes(view.state);
    const save = document.getElementById("model-modal-save");
    const selectedModel = document.getElementById("model-modal-model")?.value.trim() || "";
    if (save && RuntimeProvider.isRuntimeProvider(_selectedModalProvider())) {
      save.disabled = _modelSaveInFlight || !view.connected || !_modalRuntimeModels.includes(selectedModel);
    }
    return view;
  }

  function _applyModalRuntimeDiscovery(data) {
    _modalRuntimeModels = Array.isArray(data.models) ? data.models : [];
    const input = document.getElementById("model-modal-model");
    const existing = input.value.trim();
    const legacy = ["ChatGPT default", "Codex default"].includes(existing);
    if (!_modalRuntimeModels.includes(existing) || legacy) {
      input.value = data.default_model || _modalRuntimeModels[0] || "";
    }
    input.readOnly = true;
    _updateModalModelDropdown(_modalRuntimeModels);
    _renderModalRuntimeStatus(data);
  }

  async function _refreshModalRuntimeStatus(provider = _selectedModalProvider()) {
    if (!RuntimeProvider.isRuntimeProvider(provider)) return null;
    const request = ++_runtimeStatusRequest;
    const isCurrent = () => {
      const selected = _selectedModalProvider();
    ...
    _renderModalRuntimeStatus(null);
    let data = await RuntimeProvider.connect(provider);
    if (!isCurrent()) return data;
    let view = _renderModalRuntimeStatus(data);
    if (!view.connected && !view.terminal) {
      data = await RuntimeProvider.pollStatus(provider, {
        maxAttempts: 60,
        intervalMs: 1000,
        ...
        onUpdate: (nextData, nextView) => {
          if (isCurrent()) _renderModalRuntimeStatus(nextData, nextView);
        },
      });
      if (!isCurrent()) return data;
      view = data.view || _renderModalRuntimeStatus(data);
    }
    if (!view.connected) return data;

    const discovery = await RuntimeProvider.discover(provider);
    if (isCurrent()) _applyModalRuntimeDiscovery(discovery);
    return discovery;
  }

  async function _authenticateModalRuntime() {
    const provider = _selectedModalProvider();
    ...
    if (_modalRuntimeStatusData?.can_install === true) {
      _renderModalRuntimeStatus({ status: "starting", message: I18n.t("runtime.provider.installing") });
      const installed = await RuntimeProvider.install(provider);
      if (!isCurrent()) return;
      if (!installed.ok) {
        _renderModalRuntimeStatus(installed);
        return;
      }
      await _refreshModalRuntimeStatus(provider);
      return;
    }
    _renderModalRuntimeStatus({ status: "starting" });
    const started = await RuntimeProvider.authenticateWithBrowser(provider);
    if (!isCurrent()) return;
    if (!started.ok) {
      _renderModalRuntimeStatus(started);
      return;
    }
    const result = await RuntimeProvider.pollStatus(provider, {
      ...
      onUpdate: (data, view) => {
        if (isCurrent()) {
          _renderModalRuntimeStatus(data, view);
        }
      },
    });
    if (isCurrent()) {
      _renderModalRuntimeStatus(result, result.view);
  ...
  function _syncModalProviderMode(provider) {
    const runtime = RuntimeProvider.isRuntimeProvider(provider);
    const apiFields = document.getElementById("model-modal-api-fields");
    const runtimePanel = document.getElementById("model-modal-runtime-panel");
    ...
    if (runtime) {
      ...
      const input = document.getElementById("model-modal-model");
      input.readOnly = true;
      _refreshModalRuntimeStatus(provider);
    } else {
      _runtimeStatusRequest += 1;
      _modalRuntimeModels = [];
      const input = document.getElementById("model-modal-model");
  ...
  function _setModalSaveBusy(busy) {
    _modelSaveInFlight = busy;
    const modal = document.getElementById("model-edit-modal");
    if (modal) {
      modal.setAttribute("aria-busy", busy ? "true" : "false");
      modal.querySelectorAll("input, button, select, textarea").forEach(control => {
        if (busy) {
          if (!control.hasAttribute("data-model-save-disabled")) {
            control.dataset.modelSaveDisabled = control.disabled ? "true" : "false";
          }
          control.disabled = true;
        } else if (control.hasAttribute("data-model-save-disabled")) {
          control.disabled = control.dataset.modelSaveDisabled === "true";
          delete control.dataset.modelSaveDisabled;
        }
      });
    }

    ["model-modal-provider-trigger", "model-modal-api-format-trigger"].forEach(id => {
      const trigger = document.getElementById(id);
      if (!trigger) return;
      trigger.setAttribute("aria-disabled", busy ? "true" : "false");
      trigger.style.pointerEvents = busy ? "none" : "";
      if (busy) {
        if (!trigger.hasAttribute("data-model-save-tabindex")) {
          trigger.dataset.modelSaveTabindex = trigger.getAttribute("tabindex") || "";
        }
        trigger.setAttribute("tabindex", "-1");
        trigger.blur();
      } else if (trigger.hasAttribute("data-model-save-tabindex")) {
        const previous = trigger.dataset.modelSaveTabindex;
        if (previous) trigger.setAttribute("tabindex", previous);
        else trigger.removeAttribute("tabindex");
        delete trigger.dataset.modelSaveTabindex;
      }
    });

    ["model-modal-provider-dropdown", "model-modal-api-format-dropdown"].forEach(id => {
      const dropdown = document.getElementById(id);
      if (dropdown) dropdown.style.pointerEvents = busy ? "none" : "";
    });
    if (busy) {
      _closeProviderDropdown();
      if (_apiFormatSelect) _apiFormatSelect.close();
  ...
  function _scheduleModalSaveClose(operation, providerKey, index) {
    setTimeout(() => {
      const modal = document.getElementById("model-edit-modal");
      const currentIndex = parseInt(document.getElementById("model-modal-index").value, 10);
      if (
        operation !== _modelSaveRequest ||
        _modalProviderKey() !== providerKey ||
        currentIndex !== index ||
  ...
  function _selectModalProvider(value) {
    if (_modelSaveInFlight) {
      const selectedValue = _modalProviderIsCustom ? "custom" : (_modalSelectedProviderId || "");
      if (_providerSelect) _providerSelect.setValue(selectedValue);
      return;
    }

    const previousProviderId = _modalProviderKey();
    const previousWasRuntime = RuntimeProvider.isRuntimeProvider(_selectedModalProvider());
    ...
    const providerChanged = previousProviderId !== (value || "");
    ...
    const testResult = document.getElementById("model-modal-test-result");
    if (testResult) {
      testResult.textContent = "";
      testResult.className = "model-test-result";
    }

    if (providerChanged || previousWasRuntime !== nextIsRuntime) {
      _clearModalApiFields();
      document.getElementById("model-modal-source-id").value = "";
    }
    ...
    if (provider && !nextIsRuntime) {
      document.getElementById("model-modal-model").value = provider.default_model || "";
      document.getElementById("model-modal-baseurl").value = provider.base_url || "";
      if (provider.website_url) {
        apiKeyLink.href = provider.website_url;
        apiKeyLink.style.display = "";
      } else {
        apiKeyLink.style.display = "none";
      }
    } else if (!nextIsRuntime) {
      document.getElementById("model-modal-model").value = "";
      document.getElementById("model-modal-baseurl").value = "";
      apiKeyLink.style.display = "none";
    } else {
    ...
    _syncModalProviderMode(provider);
    setTimeout(() => {
      _updateModalModelDropdown();
      _updateModalBaseUrlDropdown();
    }, 0);
  ...
  async function _testModel(index) {
    ...
    const testBtn = document.querySelector(`[data-action="test"][data-index="${index}"]`);
    ...
    try {
      const provider = _getProvider(model);
      const result = RuntimeProvider.isRuntimeProvider(provider)
        ? await ModelTester.testRuntime({
            provider_id: provider.id,
            id: model.id,
            index: typeof model.index === "number" ? model.index : index,
          })
        : await ModelTester.testConnection({
            model:    model.model,
      ...
      if (isCurrent()) _showTestResult(index, result.ok, result.message);
    } finally {
      if (isCurrent() && testBtn) testBtn.disabled = false;
    }
  }

  async function _saveRuntimeModalModel(provider, index, saveBtn) {
    const operation = ++_modelSaveRequest;
    const existing = index < 0 ? {} : (_models[index] || {});
    const existingId = existing.id || null;
    const remark = document.getElementById("model-modal-remark").value.trim();
    const setDefault = document.getElementById("model-modal-set-default").checked;
    const selectedModel = document.getElementById("model-modal-model").value.trim();

    _setModalSaveBusy(true);
    saveBtn.disabled = true;
    saveBtn.textContent = I18n.t("settings.models.btn.testing");
    _showModalTestResult(null, "");

    const testResult = await RuntimeProvider.discover(provider);
    if (operation !== _modelSaveRequest || _selectedModalProvider()?.id !== provider.id) return;
    _showModalTestResult(testResult.ok, testResult.message);
    if (!testResult.ok || !testResult.models.includes(selectedModel)) {
      _setModalSaveBusy(false);
      saveBtn.textContent = I18n.t("settings.models.btn.save");
      saveBtn.disabled = false;
      return;
    ...
    const payload = {
      ...
      remark,
      type: setDefault ? "default" : null,
    };
    if (existingId) delete payload.provider_id;

    const saveResult = await ModelTester.saveModel(payload, { existingId });
    if (operation !== _modelSaveRequest || _selectedModalProvider()?.id !== provider.id) return;
    if (saveResult.ok) {
      saveBtn.textContent = I18n.t("settings.models.btn.saved");
      _scheduleModalSaveClose(operation, provider.id, index);
    } else {
      _setModalSaveBusy(false);
      saveBtn.textContent = I18n.t("settings.models.btn.save");
      saveBtn.disabled = false;
  ...
  function _modalProviderKindChanged(provider, index) {
    if (index < 0 || !_models[index]) return false;
    const existingIsRuntime = !!_models[index].runtime_id;
    const nextIsRuntime = RuntimeProvider.isRuntimeProvider(provider);
    if (existingIsRuntime !== nextIsRuntime) return true;
    return existingIsRuntime && _models[index].provider_id !== provider.id;
  }
  ...
  async function _saveModalModel() {
    ...
    if (_modalProviderKindChanged(provider, index)) {
      _showModalTestResult(false, I18n.t("runtime.provider.changeKind"));
      return;
    }
    if (RuntimeProvider.isRuntimeProvider(provider)) {
      await _saveRuntimeModalModel(provider, index, saveBtn);
      return;
    }

    const operation = ++_modelSaveRequest;
    const selectedProviderKey = _modalProviderKey();
    ...
    const result = await ModelTester.testConnection({
      model, base_url, api_key, index, id: existingId || sourceId, anthropic_format, api_format
    });
    if (operation !== _modelSaveRequest || _modalProviderKey() !== selectedProviderKey) return;
    ...
    const payload = { model, base_url, anthropic_format, api_format, provider_id: selectedProviderKey || null };
    ...
    const noVision = document.getElementById("model-modal-no-vision").checked;
    payload.capabilities = noVision ? { vision: false } : {};
    const setDefault = document.getElementById("model-modal-set-default").checked;
    ...
    if (saveResult.ok) {
      saveBtn.textContent = I18n.t("settings.models.btn.saved");
      _scheduleModalSaveClose(operation, selectedProviderKey, index);
    } else {
      _setModalSaveBusy(false);
      saveBtn.textContent = I18n.t("settings.models.btn.save");
      saveBtn.disabled = false;
  ...
  function _initModal() {
    ...
    document.getElementById("model-modal-runtime-login").addEventListener("click", _authenticateModalRuntime);
    document.getElementById("model-modal-runtime-recheck").addEventListener("click", () => _refreshModalRuntimeStatus());
  ...
  function _updateModalModelDropdown(modelsOverride) {
    const modelDropdown = document.getElementById("model-modal-model-dropdown");
    const modelInput = document.getElementById("model-modal-model");
    const provider = _getModalCurrentProvider();
    const runtime = RuntimeProvider.isRuntimeProvider(_selectedModalProvider());
    const models = Array.isArray(modelsOverride)
      ? modelsOverride
      : (runtime ? _modalRuntimeModels : (provider?.models || []));
  ...
  async function _removeModel(index) {
    ...
    const target = _models[index];
    const modelName = RuntimeProvider.displayModel(target, _getProvider(target)) || String(index + 1);
  ...
  async function _initSearchSettings() {
    ...
    function _showStatus(message, ok) {
      ...
      saveBtn.addEventListener("click", async () => {
        ...
        try {
          const res = await fetch("/api/config/search", {
            method:  "PATCH",
            headers: { "Content-Type": "application/json" },
            body:    JSON.stringify({ provider: select.value, key: keyInput.value })
          });
          const data = await res.json();
          if (data.ok) {
            _showStatus(I18n.t("settings.search.saved"), true);
          } else {
            _showStatus(data.error || I18n.t("settings.search.failed"), false);
          }
        } catch (e) {
          _showStatus(e.message || I18n.t("settings.search.failed"), false);
        }
      });
    }
    ...
    if (testBtn && !testBtn.dataset.bound) {
      ...
      testBtn.addEventListener("click", async () => {
        status.textContent = I18n.t("settings.search.testing");
        status.className   = "model-test-result";
        try {
          const res = await fetch("/api/config/search/test", {
            method:  "POST",
            headers: { "Content-Type": "application/json" },
            body:    JSON.stringify({ provider: select.value, key: keyInput.value })
          });
          const data = await res.json();
          _showStatus(data.message || (data.ok ? I18n.t("settings.search.ok") : I18n.t("settings.search.failed")), data.ok);
        } catch (e) {
          _showStatus(e.message || I18n.t("settings.search.failed"), false);
        }
      });
  ...
  let _loadedPlatformSource = "";
  const OFFICIAL_PLATFORM_SOURCE = "https://www.openclacky.com";

  async function _initPlatformSourceSettings() {
    const input = document.getElementById("settings-clacky-license-server");
    const saveBtn = document.getElementById("btn-save-clacky-license-server");
    const restoreBtn = document.getElementById("btn-restore-clacky-license-server");
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        _loadedPlatformSource = data.clacky_license_server || "";
        input.value = _loadedPlatformSource;
      }
    } catch (_) { /* non-critical */ }
    _syncPlatformSourceButtons();

    if (!saveBtn.dataset.bound) {
      saveBtn.dataset.bound = "1";
      saveBtn.addEventListener("click", () => _savePlatformSource());
      input.addEventListener("keydown", e => {
        if (e.key === "Enter") _savePlatformSource();
      });
    }
    if (restoreBtn && !restoreBtn.dataset.bound) {
      restoreBtn.dataset.bound = "1";
      restoreBtn.addEventListener("click", () => {
        _savePlatformSource(OFFICIAL_PLATFORM_SOURCE);
      });
    }
  }

  function _platformSourceOrigin(value) {
    try {
  ...
  function _syncPlatformSourceButtons({ busy = false } = {}) {
    const saveBtn = document.getElementById("btn-save-clacky-license-server");
    const restoreBtn = document.getElementById("btn-restore-clacky-license-server");
    if (saveBtn) saveBtn.disabled = busy;
    if (restoreBtn) {
      const official = _platformSourceOrigin(OFFICIAL_PLATFORM_SOURCE);
      restoreBtn.disabled = busy ||
        _platformSourceOrigin(_loadedPlatformSource) === official;
    }
  }

  async function _savePlatformSource(sourceOverride = null) {
    const input = document.getElementById("settings-clacky-license-server");
    const saveBtn = document.getElementById("btn-save-clacky-license-server");
    const status = document.getElementById("settings-clacky-license-server-status");
    ...
    status.className = "model-test-result";
    if (!source) {
      status.textContent = I18n.t("settings.brand.source.required");
      status.className = "model-test-result result-fail";
      return;
    }

    const sourceChanged =
      _platformSourceOrigin(source) !== _platformSourceOrigin(_loadedPlatformSource);
    if (sourceChanged) {
      _syncPlatformSourceButtons({ busy: true });
      const confirmed = await Modal.confirm(I18n.t("settings.brand.source.confirm"));
      if (!confirmed) {
        _syncPlatformSourceButtons();
        return;
      }
    ...
    _syncPlatformSourceButtons({ busy: true });
    saveBtn.textContent = I18n.t("settings.brand.source.saving");

    try {
      const res = await fetch("/api/config/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ clacky_license_server: source })
      });
      const data = await res.json();
      if (!res.ok || !data.ok) {
        throw new Error(data.error || I18n.t("settings.brand.source.invalid"));
      }
      ...
      if (data.restarting) {
        status.textContent = I18n.t("settings.brand.source.reconnecting");
        status.className = "model-test-result result-ok";
        const reconnected = await _waitForPlatformSourceRestart();
        if (!reconnected) {
          throw new Error(I18n.t("settings.brand.source.reconnectFailed"));
        }
      }

      await Brand.refresh();
      status.textContent = I18n.t("settings.brand.source.saved");
      status.className = "model-test-result result-ok";
    } catch (e) {
      status.textContent = e.message || I18n.t("settings.brand.source.invalid");
      status.className = "model-test-result result-fail";
    } finally {
      _syncPlatformSourceButtons();
      saveBtn.textContent = I18n.t("settings.brand.source.save");
    }
  }

  async function _waitForPlatformSourceRestart() {
    // The old Worker can still answer HTTP while the Master is shutting it
    // down, so an HTTP success does not prove the new source is active. A full
    // WebSocket disconnect/reconnect cycle identifies the replacement Worker.
    let disconnected = !WS.ready;
    for (let attempt = 0; attempt < 40; attempt += 1) {
      if (!WS.ready) disconnected = true;
      if (disconnected && WS.ready) return true;
  ...
  function _renderBrandStatus(data) {
    ...
    if (data.branded && !data.needs_activation) {
      ...
      // Keep the activation prompt in sync when platform branding changes.
      const desc = activateForm.querySelector(".brand-activate-desc");
      if (desc) {
        desc.textContent = data.product_name
          ? I18n.t("settings.brand.descNamed", { name: data.product_name })
          : I18n.t("settings.brand.desc");
      }
  ...
  async function _onMediaSourceClick(kind, source) {
    ...
    if (source === "custom") {
      if (cur.source !== "custom" && !_mediaCustomDraft[kind]) {
        const sc = cur.saved_custom;
        _mediaCustomDraft[kind] = {
          model:    (sc && sc.model) || "",
          base_url: (sc && sc.base_url) || "",
          api_key:  cur.api_key_masked || ""
        };
      }
