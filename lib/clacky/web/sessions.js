...
const Sessions = (() => {
  ...
  function _updateInputBehavior(status = Sessions.find(Sessions.activeId)?.status) {
    ...
    const mode = $("input-behavior")?.value || "interrupt";
    ...
    const label = I18n.t(mode === "steer" ? "chat.input.steerMode" : "chat.input.interruptMode");
  ...
  async function _sendMessage() {
    ...
    const inputBehavior = $("input-behavior")?.value || "interrupt";
    const queuing = inputBehavior === "steer";
    if (!queuing) {
  ...
  function _makeToolItem(name, args, summary, toolCallId = null) {
    const item = document.createElement("div");
    ...
    if (toolCallId) item.dataset.toolCallId = String(toolCallId);
  ...
  function _addToolCallToGroup(group, name, args, summary, toolCallId = null) {
    const body   = group.querySelector(".tool-group-body");
    const header = group.querySelector(".tool-group-header");
    const count  = group.querySelector(".tg-count");
    const item   = _makeToolItem(name, args, summary, toolCallId);
  ...
  function _findToolItemByCallId(container, toolCallId) {
    if (!container || !toolCallId) return null;
    const id = String(toolCallId);
    const items = Array.from(container.querySelectorAll(".tool-item"));
    for (let i = items.length - 1; i >= 0; i--) {
      if (items[i].dataset.toolCallId === id) return items[i];
    }
    return null;
  }
  ...
  function _completeToolItem(last, result, {
    collapsed = false,
    ...
    const numericExitCode = exitCode == null || exitCode === "" ? null : Number(exitCode);
    const failed = resultStatus === "failed" ||
      (Number.isFinite(numericExitCode) && numericExitCode !== 0);
    if (status) {
      status.className = `tool-item-status ${failed ? "err" : "ok"}`;
    ...
    if (stdout) {
      ...
      if (result && typeof result === "object") {
        renderedResult = result.formatted_output ?? result.formattedOutput ??
          result.output ?? result.content;
        if (renderedResult == null) {
          try { renderedResult = JSON.stringify(result); } catch (_) { renderedResult = String(result); }
        }
      }
      let resultStr = (renderedResult == null) ? "" : String(renderedResult).trim();
      if (!resultStr && failed && Number.isFinite(numericExitCode)) {
        resultStr = `Exit code ${numericExitCode}`;
      }
  ...
  function _renderHistoryEvent(ev, outerContainer, historyCtx) {
    ...
    switch (ev.type) {
      ...
      case "tool_call": {
        ...
        historyCtx.lastItem = _addToolCallToGroup(
          historyCtx.group, ev.name, ev.args, ev.summary, ev.tool_call_id
        );
        break;
      }

      case "tool_result": {
        const keyedItem = ev.tool_call_id
          ? _findToolItemByCallId(container, ev.tool_call_id)
          : null;
        const item = keyedItem || historyCtx.lastItem;
        if (item) _completeToolItem(item, ev.result, {
          collapsed: true,
          status: ev.status,
  ...
  return {
    ...
    async fork(sessionId) {
      ...
      if (session && session.runtime_id) return;
    ...
    _showActionsMenu(button, session) {
      ...
      const forkItemHtml = session.runtime_id ? "" : `
        <div class="session-actions-menu-item" data-action="fork">
          <span class="session-actions-menu-icon">${iconFork}</span>
          <span class="session-actions-menu-label">${escapeHtml(I18n.t("sessions.actions.fork"))}</span>
        </div>`;
    ...
    updateInfoBar(s) {
      const previousSessionId = this._lastSession && this._lastSession.id;
      this._lastSession = s;
      if (!s || previousSessionId !== s.id) {
        SessionModelSwitcher.close();
      }
      if (!s || previousSessionId !== s.id || s.runtime_id) {
        ReasoningEffortSwitcher.close();
      }
      ...
      if (sibModel) {
        ...
        if (s.runtime_id) sibModel.dataset.runtimeId = s.runtime_id; else delete sibModel.dataset.runtimeId;
      ...
      if (sibReasoning) {
        const reportedEffort = s.reasoning_effort == null
          ? ""
          : String(s.reasoning_effort).trim();
        const eff = (reportedEffort || "off").toLowerCase();
        const reasoningMutable = !s.runtime_id;
        sibReasoning.textContent = !reasoningMutable && !reportedEffort
          ? I18n.t("sib.reasoning.pending")
          : I18n.t(`sib.reasoning.${eff}`);
        sibReasoning.dataset.sessionId = s.id;
        sibReasoning.dataset.reasoningEffort = eff;
        sibReasoning.dataset.reasoningMutable = String(reasoningMutable);
        sibReasoning.classList.toggle("sib-reasoning-disabled", !reasoningMutable);
        sibReasoning.setAttribute("aria-disabled", String(!reasoningMutable));
        sibReasoning.title = reasoningMutable
          ? I18n.t("sib.reasoning.tooltip")
          : I18n.t("sib.reasoning.runtimeManaged");
      }
      if (sibReasoningWrap) sibReasoningWrap.style.display = "";
      if (sibSepAfterReasoning) sibSepAfterReasoning.style.display = "";

      const slashButton = $("btn-slash");
      if (slashButton) slashButton.hidden = !!s.runtime_id;
      if (s.runtime_id && typeof SkillAC !== "undefined") SkillAC.hide();
    ...
    appendToolCall(name, args, summary, toolCallId = null) {
      const messages = RenderTarget.current();
      const ctx = Sessions._toolGroupCtx(messages);
      const keyedItem = toolCallId ? _findToolItemByCallId(messages, toolCallId) : null;
      if (keyedItem) {
        ctx.item = keyedItem;
        return;
      }
      ...
      ctx.item = _addToolCallToGroup(ctx.group, name, args, summary, toolCallId);
      _scrollToBottomIfNeeded(messages);
    },
    ...
    appendToolResult(result, toolCallId = null, options = {}) {
      const messages = RenderTarget.current();
      const ctx = Sessions._toolGroupCtx(messages);
      const keyedItem = toolCallId ? _findToolItemByCallId(messages, toolCallId) : null;
      if (keyedItem) {
        _completeToolItem(keyedItem, result, options);
        if (ctx.item === keyedItem) ctx.item = null;
        return;
      }
      if (ctx.group && ctx.item) {
        _completeLastToolItem(ctx.group, result, options);
        return;
      }
      if (messages) {
        ...
        if (running.length > 0) {
          ...
          if (item) { _completeToolItem(item, result, options); return; }
        }
      }
    },
    ...
    appendAssistantMessage(content, { messageId = null, messageIds = null, delta = false } = {}) {
      if (!messageId) {
      ...
      const messages = RenderTarget.current();
      const assistantStreams = messages._assistantStreams ||
        (messages._assistantStreams = new Map());
      let element = assistantStreams.get(messageId);
      if (element && !element.isConnected) {
        assistantStreams.delete(messageId);
        element = null;
      }
      ...
      if (delta) {
        Sessions.collapseToolGroup();
        if (!element) {
          element = Sessions.appendMsg("assistant", "");
          assistantStreams.set(messageId, element);
        }
        content = (element.dataset.raw || "") + (content || "");
      } else if (!element) {
        Sessions.appendMsg("assistant", content);
        return;
      }
      ...
      element.innerHTML = _renderMarkdown(content || "");
      _appendCopyButton(element);
      _enhanceTaskItems(element, content || "");
      _scrollToBottomIfNeeded(messages);
      if (!delta) {
        for (const id of Array.isArray(messageIds) ? messageIds : []) {
          if (String(id) === String(messageId)) continue;
          const extraElement = assistantStreams.get(String(id));
          if (extraElement && extraElement !== element) extraElement.remove();
          assistantStreams.delete(String(id));
        }
        assistantStreams.delete(messageId);
      }
    },
...
// + session-actions dropdown). Self-contained modules bind themselves
// on document (event delegation), so no explicit init() call is needed —
// they just work once this file is loaded.
//
// Moved here from app.js; each module preserves its private state
// (benchmark cache, open/closed flags) without polluting the Sessions closure.
// ─────────────────────────────────────────────────────────────────────────

// ── Session Info Bar Model Switcher ───────────────────────────────────────
const SessionModelSwitcher = (() => {
  let _isOpen = false;
  let _isOpening = false;
  let _requestVersion = 0;

  function _isCurrent(requestVersion, sessionId) {
    return requestVersion === _requestVersion && Sessions.activeId === sessionId;
  }

  function _isCurrentAnchor(modelEl, sessionId) {
    return !!modelEl && modelEl.isConnected &&
      modelEl.dataset.sessionId === sessionId &&
      !modelEl.classList.contains("sib-model-disabled");
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (modelEl) {
      ...
      if (_isOpen || _isOpening) {
        ...
        const sessionId = modelEl.dataset.sessionId;
        if (!sessionId || Sessions.activeId !== sessionId ||
            !_isCurrentAnchor(modelEl, sessionId)) return;
        let subOptions = [];
        ...
        const subInfo = {
          ...
          runtimeId: modelEl.dataset.runtimeId || null
        };
        const requestVersion = ++_requestVersion;
        _isOpening = true;
        const populated = await _populateModelDropdown(
          sessionId,
          modelEl.dataset.modelId || null,
          subInfo,
          requestVersion
        );
        if (!populated || !_isCurrent(requestVersion, sessionId) ||
            !_isCurrentAnchor(modelEl, sessionId)) {
          if (requestVersion === _requestVersion) _isOpening = false;
          return;
        }
        _isOpening = false;
    ...
    if ((_isOpen || _isOpening) && !e.target.closest(".sib-model-dropdown") && !e.target.closest(".sib-submodel-panel")) {
      close();
    }
  });

  // Populate dropdown with available models
  async function _populateModelDropdown(sessionId, currentModelId, subInfo, requestVersion) {
    ...
    try {
      ...
      if (!_isCurrent(requestVersion, sessionId)) return false;
      const data = await res.json();
      if (!_isCurrent(requestVersion, sessionId)) return false;
      if (!res.ok) throw new Error(data.error || I18n.t("sib.model.unknownError"));

      const sessionModel = data.session_model && typeof data.session_model === "object"
        ? data.session_model
        : null;
      const models = Array.isArray(data.models) ? data.models.slice() : [];
      const mediaCaps = data.media_capabilities || {};
      const effectiveCurrentId = (sessionModel && sessionModel.id) || currentModelId;
      const runtimeId = (sessionModel && sessionModel.runtime_id) || subInfo.runtimeId;
      const currentIsRuntime = !!runtimeId;
      const effectiveSubInfo = {
        options: sessionModel && Array.isArray(sessionModel.sub_model_options)
          ? sessionModel.sub_model_options
          : subInfo.options,
        current: sessionModel && Object.prototype.hasOwnProperty.call(sessionModel, "sub_model")
          ? sessionModel.sub_model
          : subInfo.current,
        cardModel: (sessionModel && sessionModel.card_model) || subInfo.cardModel
      };

      if (sessionModel && sessionModel.id && !models.some(model => model.id === sessionModel.id)) {
        models.unshift(sessionModel);
      }

      if (models.length === 0) {
        dropdown.innerHTML = `<div style="padding:0.75rem;text-align:center;color:var(--color-text-secondary);font-size:0.6875rem;">${I18n.t("sib.model.empty")}</div>`;
        return true;
      }
      ...
      ModelPicker.populate(dropdown, {
        ...
        mediaCaps,
        subInfo: effectiveSubInfo,
        isSelectable: (m) => currentIsRuntime ? m.id === effectiveCurrentId : !m.runtime_id,
        onSelect: (m) => {
          if (m.id === effectiveCurrentId) {
            close();
            return;
          }
          return _switchModel(sessionId, m.id, RuntimeProvider.displayModel(m));
        },
        onSwitchSubModel: (name, displayName) => _switchSubModel(sessionId, name, displayName),
        onBenchmark: currentIsRuntime ? null : () => _benchmarkSession(sessionId),
      ...
      dropdown.innerHTML = `<div style="padding:0.75rem;text-align:center;color:var(--color-error);font-size:0.6875rem;">${I18n.t("sib.model.loadError")}</div>`;
      return true;
    }
  }
  ...
  async function _benchmarkSession(sessionId) {
    ...
    if (!res.ok || !data.ok) throw new Error(data.error || I18n.t("sib.bench.failed", { msg: "" }));
    return data.results;
  }
  ...
  async function _switchModel(sessionId, modelId, modelName) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || I18n.t("sib.model.unknownError"));
      }
      ...
      alert(I18n.t("sib.model.switchError", { message: e.message }));
    }
  }
  ...
  async function _switchSubModel(sessionId, modelName, displayName) {
    ...
    try {
      ...
      alert(I18n.t("sib.model.switchSubmodelError", { message: e.message }));
    }
  }
...
const ReasoningEffortSwitcher = (() => {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (el) {
      e.stopPropagation();
      if (el.dataset.reasoningMutable !== "true") return;
      const dropdown = $("sib-reasoning-dropdown");
      if (!dropdown) return;
