...
const Onboard = (() => {
  ...
  function _showSetupStep(step) {
    ...
    if (step === "key") {
      // A branded client skips the public AI Keys promotion, but enterprise
      // device login remains available. Manual BYOK stays as the fallback.
  ...
  function _bindDeviceStep() {
    ...
    const enterpriseCancel = $("setup-enterprise-cancel");
    if (enterpriseCancel) enterpriseCancel.addEventListener("click", _restoreDeviceChoices);

    const enterpriseSubmit = $("setup-enterprise-submit");
    if (enterpriseSubmit) enterpriseSubmit.addEventListener("click", _startEnterpriseDeviceLogin);

    const cancel = $("setup-device-cancel");
    if (cancel) cancel.addEventListener("click", () => {
      _devicePolling = false;
      _restoreDeviceChoices();
    });
  ...
  function _showDevicePending(on) {
    ...
    const enterpriseLink = $("setup-btn-enterprise-login");
    const enterpriseForm = $("setup-enterprise-form");
    if (pending) pending.style.display = on ? "" : "none";
    if (card) card.style.display = on ? "none" : (_branded ? "none" : "");
    if (enterpriseLink) enterpriseLink.style.display = on ? "none" : "";
    if (enterpriseForm) enterpriseForm.style.display = "none";
    $("setup-manual-toggle").style.display = on ? "none" : "";
    $("setup-manual-section").style.display = "none";
  }

  function _showDeviceSuccess(model, enterprise = false, restarting = false) {
    const pending = $("setup-device-pending");
    const card    = $("setup-device-card");
    const enterpriseLink = $("setup-btn-enterprise-login");
    const enterpriseForm = $("setup-enterprise-form");
    ...
    const lead = $("setup-device-success-lead");
    const credit = $("setup-device-success-credit");
    const note = $("setup-device-success-note");
    const modelLabel = $("setup-device-success-model-label");
    if (pending) pending.style.display = "none";
    if (card)    card.style.display    = "none";
    if (enterpriseLink) enterpriseLink.style.display = "none";
    if (enterpriseForm) enterpriseForm.style.display = "none";
    $("setup-manual-toggle").style.display = "none";
    $("setup-manual-section").style.display = "none";
    ...
    if (enterprise) {
      if (lead) lead.textContent = I18n.t(restarting
        ? "onboard.enterprise.success.restarting"
        : "onboard.enterprise.success.lead");
      if (credit) credit.style.display = "none";
      if (note) note.style.display = "none";
      if (modelLabel) modelLabel.textContent = I18n.t("onboard.enterprise.success.model");
    }
  }
  ...
  async function _startEnterpriseDeviceLogin() {
    let platformSource = $("setup-enterprise-source").value.trim();
    const zh = _selectedLang === "zh";
    if (!platformSource) {
      _setDeviceError(zh ? "请输入企业网页地址。" : "Enter your enterprise website address.");
      return;
    }
    if (!/^https?:\/\//i.test(platformSource)) platformSource = "https://" + platformSource;
    $("setup-enterprise-source").value = platformSource;
    await _startDeviceLogin(platformSource);
  }

  async function _startDeviceLogin(platformSource = null) {
    const zh = _selectedLang === "zh";
    _deviceLoginEnterprise = !!platformSource;
    _setDeviceError("");
    ...
    try {
      ...
      if (platformSource) {
        ...
        requestOptions.body = JSON.stringify({ platform_source: platformSource });
      }
      const res = await fetch("/api/onboard/device/start", requestOptions);
      data = await res.json();
    } catch (_) {
      data = null;
    }
    ...
    _pollDevice(
      ...
      data.platform_source || platformSource
    );
  }

  async function _pollDevice(deviceCode, intervalMs, platformSource) {
    const zh = _selectedLang === "zh";
    ...
    while (_devicePolling) {
      ...
      if (data.status === "approved") {
        _devicePolling = false;
        _showDeviceSuccess(data.default_model, _deviceLoginEnterprise, data.restarting);
        return;
      }
