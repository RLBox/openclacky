...
const Onboard = (() => {
  ...
  async function _authenticateSetupRuntime() {
    ...
    if (_setupRuntimeStatusData?.can_install === true) {
      _renderSetupRuntimeStatus({ status: "starting", message: I18n.t("runtime.provider.installing") });
      const installed = await RuntimeProvider.install(provider);
      if (!isCurrent()) return;
      if (!installed.ok) {
        _renderSetupRuntimeStatus(installed);
        return;
      }
      await _refreshSetupRuntimeStatus(provider);
      return;
    }
  ...
  function _syncSetupProviderMode(provider) {
    ...
    const hasSelection = !!provider || _selectedProviderId === "__custom__";
    const modelField = $("setup-model-field");
    const apiFields = $("setup-api-fields");
    const runtimePanel = $("setup-runtime-panel");
    const continueBtn = $("setup-btn-test");
    // Fail closed while no provider is selected: stale provider values must
    // never look like a valid API configuration on the welcome screen.
    if (modelField) modelField.style.display = hasSelection ? "" : "none";
    if (apiFields) apiFields.style.display = hasSelection && !runtime ? "" : "none";
    if (runtimePanel) runtimePanel.style.display = runtime ? "" : "none";
    if (continueBtn) {
      continueBtn.disabled = !hasSelection || runtime;
