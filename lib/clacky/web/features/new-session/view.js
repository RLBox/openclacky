...
const NewSessionView = (() => {
  ...
  async function _populateModels() {
    ...
    // Removed/disabled runtime extensions leave their saved cards visible so
    // users can repair or delete them in Settings, but a new session must use
    // a currently available card.
    const selected = models.find((m) => m.id === NewSessionStore.state.advanced.modelId);
    if (!selected || selected.runtime_available === false) {
      const available = models.filter((m) => m.runtime_available !== false);
      const def = available.find((m) => m.type === "default") || available[0];
      if (def) NewSessionStore.updateAdvanced({ modelId: def.id || "" });
      else NewSessionStore.updateAdvanced({ modelId: "" });
    }
    _renderContextBar();
  }
  ...
  function _updateSendButton() {
    ...
    const selectedModel = NewSessionStore.state.models.find(
      (m) => m.id === NewSessionStore.state.advanced.modelId
    );
    const modelAvailable = !selectedModel || selectedModel.runtime_available !== false;
    btn.disabled = (!hasContent && !hasFiles && !initProject) ||
      NewSessionStore.state.creating || !modelAvailable;
  }
  ...
  function _openModelPopover(anchor) {
    _showCtxPopover(anchor, (pop) => {
      ...
      ModelPicker.populate(pop, {
        models,
        currentId,
        isSelectable: (m) => m.runtime_available !== false,
        onSelect: (m) => {
  ...
  function _renderContextBar() {
    ...
    if (modelId) {
      ...
      if (found) {
        const raw = RuntimeProvider.displayModel(found);
