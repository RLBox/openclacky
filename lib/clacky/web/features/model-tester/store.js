...
window.RuntimeProvider = (function () {
  ...
  // The Codex authentication request is allowed to wait for five minutes.
  ...
  function install(provider) {
    return runtimeRequest(provider, "install", { method: "POST" });
  }
  ...
  async function authenticateWithBrowser(provider) {
    // Reserve the tab inside the click event. Safari may block window.open
    // after the asynchronous authentication request has completed.
    ...
    if (loginWindow) {
      ...
      loginWindow.location.replace(result.auth_url);
    } else {
