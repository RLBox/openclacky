...
RSpec.describe Clacky::Server::HttpServer, "runtime session lifecycle" do
  ...
  # Runtime integration tests inject isolated registries without widening the
  # shared HTTP-server helper used by the rest of the suite.
  def with_server(agent_config:, client_factory: -> { double("client") },
                  sessions_dir: nil, projects_file: nil,
                  provider_registry: nil, runtime_registry: nil)
    dir = sessions_dir || Dir.mktmpdir("clacky_http_runtime_spec_sessions")
    projects_dir = Dir.mktmpdir("clacky_http_runtime_spec_projects") unless projects_file
    ...
    FileUtils.rm_rf(dir) unless sessions_dir
  ...
  it "cancels a running Codex turn and drains its replacement only after the prompt response" do
    barrier_runtimes = []
    ...
    registry = Clacky::AgentRuntimeRegistry.new(
      factories: { "codex" => barrier_factory }
    )
  ...
  it "keeps an explicitly cancelled runtime worker alive until the Codex response barrier" do
    barrier_runtimes = []
    barrier_factory = lambda do |**options|
      BarrierRuntimeServerSpecAdapter.new(**options).tap do |runtime|
        barrier_runtimes << runtime
