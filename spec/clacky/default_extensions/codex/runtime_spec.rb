...
RSpec.describe Clacky::DefaultExtensions::Codex::Connection do
  ...
  let(:home_result) do
    OpenStruct.new(managed_home: "/managed/codex", auth_reused: true, auth_reason: "reused")
  end
  ...
  def build_connection
    described_class.new(
      ...
      launcher_factory: ->(_managed_home) { launcher },
      client_factory: ->(_launch) { client }
    )
  end
  ...
  it "allows the pinned npx fallback enough time for its first package resolution" do
    ...
    expect(client.start_arguments[:timeout]).to eq(180)
  ensure
    connection&.close
  end
...
RSpec.describe Clacky::DefaultExtensions::Codex::Runtime do
  ...
  it "falls back to a new ACP session with an explicit warning when resume is missing" do
    client.request_handler = lambda do |method, _params, _timeout|
      case method
      when "session/resume"
        raise Clacky::Acp::Client::ProtocolError, "unknown session"
      when "session/new"
